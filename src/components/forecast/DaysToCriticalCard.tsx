    ...
    <div className="rounded-2xl border border-stone-200 bg-white p-6 shadow-subtle sm:p-7 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-stone-100 pb-4">
        <div>
          <span className="text-xs font-extrabold uppercase tracking-wider text-rose-700 flex items-center gap-1.5">
            ...
            Vulnerability Time-Horizon
          </span>
          <h2 className="mt-1 text-xl sm:text-2xl font-black text-stone-900">
            Days-to-Critical Projection Indicator
          </h2>
          <p className="text-xs sm:text-sm text-stone-600 mt-1">
            Estimated time before groundwater depth reaches critical extraction limits if current depletion rates continue.
          </p>
        </div>

        <span className="rounded-full bg-stone-100 px-3 py-1 text-xs font-semibold text-stone-600 self-start sm:self-auto">
          5,260 Observation Wells Evaluated
        </span>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs font-semibold text-stone-600">
          <span>National Network Breakdown by Days Remaining</span>
          <span className="font-mono text-stone-800">4 Severity Brackets</span>
        </div>

        <div
          className="flex h-4 w-full overflow-hidden rounded-full bg-stone-100 p-0.5 border border-stone-200"
          role="progressbar"
          aria-label="Days to critical distribution"
        >
          {brackets.map((b) => {
            ...
            return (
              <div
                ...
                title={`${b.range} (${b.label}): ${b.count} stations (${b.percentage}%)`}
              />
            );
          })}
        </div>
              ...
              <div>
                ...
                <div className="mt-3">
                  ...
                  <p className="text-xs text-stone-500 font-medium mt-0.5">
                    <strong className="text-stone-900 font-bold">{b.count}</strong> wells ({b.percentage}%)
                  </p>
                </div>
      ...
      <div className="flex items-start gap-2 rounded-xl bg-stone-50 p-3 text-xs text-stone-600 border border-stone-200">
        ...
        <p>
          <strong className="text-stone-800">Forecast Note: </strong>
          Days-to-critical estimates are calculated from current hydrostatic drawdown slope and historical monsoon infiltration response. Projections adapt dynamically as seasonal rainfall and pumping patterns change.
        </p>
      </div>
    </div>
  );
