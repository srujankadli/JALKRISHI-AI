...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-300 bg-stone-50/50 p-8 text-center sm:p-12">
      ...
      <h3 className="mt-4 text-base font-bold text-stone-900 sm:text-lg">{t(title)}</h3>
      <p className="mt-1.5 max-w-sm text-xs text-stone-500 sm:text-sm">{t(description)}</p>
      {actionText && onAction && (
        <div className="mt-5">
...
export const LoadingState: React.FC<{ message?: string }> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-12 text-center">
      ...
      <p className="mt-4 text-sm font-medium text-stone-600 animate-pulse">{t(message)}</p>
    </div>
  );
};
